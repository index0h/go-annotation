...
func TestFuncSpec_Validate_WithInvalidTypeParamSpec(t *testing.T) {
	...
	ctrl.Subtest("").
		Call(model.Validate).
		ExpectPanic(NewErrorMessageConstraint("Variable 'Spec' has invalid type: %T", model.Params[0].Spec))
}
...
func TestFuncSpec_Validate_WithEmptyParamsAndIsVariadic(t *testing.T) {
	...
	ctrl.Subtest("").
		Call(model.Validate).
		ExpectPanic(NewErrorMessageConstraint("Variable 'Params' must be not empty for variadic %T", model))
}
...
func TestFuncSpec_Validate_WithInvalidTypeParamSpecAndIsVariadic(t *testing.T) {
	...
	ctrl.Subtest("").
		Call(model.Validate).
		ExpectPanic(
			NewErrorMessageConstraint("Variable 'Params[0].Spec' has invalid type for variadic %T", model),
		)
}
...
func TestFuncSpec_Validate_WithInvalidTypeResultSpec(t *testing.T) {
	...
	ctrl.Subtest("").
		Call(model.Validate).
		ExpectPanic(NewErrorMessageConstraint("Variable 'Spec' has invalid type: %T", model.Results[0].Spec))
}
...
func TestFuncSpec_Clone(t *testing.T) {
	...
	ctrl.AssertNotSame(model.Params[0].Annotations[0], actual.(*FuncSpec).Params[0].Annotations[0])
	ctrl.AssertSame(clonedFuncResultSpec, actual.(*FuncSpec).Results[0].Spec)
	ctrl.AssertNotSame(model.Results[0].Annotations[0], actual.(*FuncSpec).Results[0].Annotations[0])
}
